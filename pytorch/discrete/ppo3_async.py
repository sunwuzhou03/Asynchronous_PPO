...
class Distributions():
    ...
    def logprob(self, probs, value_data):
        distribution = Categorical(probs)
        return distribution.log_prob(value_data).unsqueeze(1).float().to(self.device)

    def kl_divergence(self, probs1, probs2):
        distribution1 = Categorical(probs1)
        distribution2 = Categorical(probs2)
...
class TrajPoolManager:
    ...
    def get_sample_utilization(self):
        return self.use_cnt/self.total_cnt
...
def main():
    ...
    start = time.time()
    ray.init()
    TrajectoryPool=TrajPoolManager.remote(capacity=20)
