...
class Distributions():
    ...
    def kl_divergence(self, logits1, logits2):
        distribution1 = Categorical(logits=logits1)
        distribution2 = Categorical(logits=logits2)
...
class Learner():
    ...
    # Loss for PPO
    def get_loss(self, logprobs, values, old_logprobs, old_values, next_values, actions, rewards, dones,entropy):
    ...
    def training_ppo(self, states, actions, rewards, dones, next_states,old_logprobs,old_values):
        ...
        loss = self.get_loss(logprobs, values, old_logprobs, old_values, next_values, actions, rewards, dones,entropy)
...
class Agent:
    ...
    def act(self, state):
        ...
        return int(action.cpu().item()),logprob.flatten(),value.flatten()
...
class TrajPoolManager:
    ...
    def get_sample_utilization(self):
        su=self.use_cnt/self.total_cnt
